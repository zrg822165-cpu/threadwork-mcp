...
import { booleanArg, requiredString, stringArg, stringArrayArg } from "./argHelpers.js";
import { removeAgentFile, slugifyAgentId, writeAgentFile } from "./agentFileService.js";
import { registerGeneratedAgent, unregisterGeneratedAgent } from "./scaffoldService.js";
import { openCodeBuilds, requireBuild, selectTeam } from "./teamStateHelpers.js";

export function teamStart(state: TeamState, args: Record<string, unknown>): unknown {
  const teamName = stringArg(args.teamName) ?? stringArg(args.name) ?? "OpenCode Team";
  ...
  const builds = openCodeBuilds(state);
  if (builds[team.id]) {
...
export function teamDraftMember(state: TeamState, args: Record<string, unknown>): TeamMemberDraft {
  const build = requireBuild(state, stringArg(args.teamId));
...
export async function teamConfirmMember(state: TeamState, rootDir: string, args: Record<string, unknown>): Promise<unknown> {
  const build = requireBuild(state, stringArg(args.teamId));
  resumeBuilding(build);
  ...
  const agentPath = await writeAgentFile({ rootDir, draft, force: booleanArg(args.force) });
  ...
  await registerGeneratedAgent(rootDir, member);
...
export function teamConfirmRuntimeMember(state: TeamState, args: Record<string, unknown>): unknown {
  const build = requireBuild(state, stringArg(args.teamId));
  resumeBuilding(build);
  const draft = build.currentDraft;
  if (!draft) {
...
export async function teamRemoveMember(state: TeamState, rootDir: string, args: Record<string, unknown>): Promise<unknown> {
  const build = requireBuild(state, stringArg(args.teamId));
  if (build.currentDraft) {
  ...
  resumeBuilding(build);
  await unregisterGeneratedAgent(rootDir, removedMember.agentId);
  const agentPath = await removeAgentFile(rootDir, removedMember.agentId);
  build.updatedAt = nowIso();
...
export function teamRemoveRuntimeMember(state: TeamState, args: Record<string, unknown>): unknown {
  const build = requireBuild(state, stringArg(args.teamId));
  if (build.currentDraft) {
    throw new ConflictError("Cannot remove a confirmed member while a draft is pending. Confirm or discard the draft first.", { draft: build.currentDraft });
  }
...
export function teamFinish(state: TeamState, args: Record<string, unknown>): unknown {
  ...
  build.updatedAt = now;
  const reportPrompts = createReportPrompts(state, build.teamId);
  return {
    build,
    reportPrompts,
    ...
    reportingInstructions:
      "Send each @member prompt in order. If OpenCode cannot resolve a newly created @member, restart or refresh OpenCode so it reloads .opencode/agents, then send the prompts again. Do not start task work during reporting."
  };
}

export function teamReportPrompts(state: TeamState, requestedTeamId?: string): unknown {
  ...
  return {
    ...
    membersToMention: reportPrompts.map((prompt) => prompt.agentId ?? prompt.memberId),
    restartMayBeRequired: true,
    reportingInstructions: "Send each @member prompt in order. Restart or refresh OpenCode if a new member cannot be mentioned."
  };
}
...
export interface TeamStatusOptions {
  rootDir?: string;
  scaffold?: unknown;
  agentFiles?: Array<{ memberId: string; agentId?: string; path?: string; exists: boolean }>;
}

export function teamBuildStatus(state: TeamState, requestedTeamId?: string, options: TeamStatusOptions = {}): unknown {
  const build = requireBuild(state, requestedTeamId);
  ...
  return {
    ...
    diagnostics: {
      ...
      restartMayBeRequired: build.confirmedMemberIds.length > 0,
      restartHint:
        build.confirmedMemberIds.length > 0
    ...
    reportPrompts: build.status === "finalized" ? reportPrompts : [],
    membersToMention: build.status === "finalized" ? reportPrompts.map((prompt) => prompt.agentId ?? prompt.memberId) : [],
    nextPrompt:
      build.status === "finalized"
        ? "Team is finalized. You can send the report prompts, or add/remove members and finish again."
        : build.currentDraft
...
function resolveConfirmedMember(
  ...
  const member = members.find((candidate) => {
    if (memberId && candidate.id === memberId) {
      return true;
    }
    if (agentId && candidate.agentId?.toLowerCase() === agentId) {
      return true;
    }
    if (name && candidate.name.trim().toLowerCase() === name) {
      return true;
    }
    return false;
  });
